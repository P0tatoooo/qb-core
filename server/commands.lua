...
QBCore.Commands.Add('me', Lang:t("command.me.help"), {{name = Lang:t("command.me.params.message.name"), help = Lang:t("command.me.params.message.help")}}, false, function(source, args)
    ...
    for i=1, #Players do
        ...
        if target == ped or #(pCoords - tCoords) < 20 then
            TriggerClientEvent('QBCore:Command:ShowMe3D', Player, source, txt)
        end
    end
    local xPlayer = QBCore.Functions.GetPlayer(source)
    TriggerEvent('MyCity_Core:MeCommand:Logs', QBCore.Functions.GetPlayerName(source), txt, xPlayer.PlayerData.source)
end, 'user')
