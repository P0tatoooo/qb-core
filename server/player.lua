...
function QBCore.Player.CheckPlayerData(source, PlayerData)
    ...
    PlayerData.charinfo.phone = PlayerData.phone or QBCore.Functions.CreatePhoneNumber()
    ...
    --Tattoos
    PlayerData.tattoos = PlayerData.tattoos or {}

    --Furnitures
    PlayerData.furnitures = PlayerData.furnitures or {}

    --Property
    PlayerData.currentproperty = PlayerData.currentproperty or ''
    ...
    PlayerData.job.label = PlayerData.job.label or 'Sans Emploi'
    ...
    PlayerData.job.grade.level = PlayerData.job.grade.level or {}
    PlayerData.job.grade.name = PlayerData.job.grade.name or 'Chômeur'
    ...
    PlayerData.gang.name = PlayerData.gang.name or 'civil'
    PlayerData.gang.label = PlayerData.gang.label or 'Civil'
    PlayerData.gang.isboss = PlayerData.gang.isboss or false
    PlayerData.gang.grade = PlayerData.gang.grade or {}
    PlayerData.gang.grade.name = PlayerData.gang.grade.name or 'Sans Affiliation'
...
function QBCore.Player.CreatePlayer(PlayerData, Offline)
    ...
    function self.Functions.SetJob(job, grade)
        ...
        if QBCore.Shared.Jobs[job].grades[grade] then
            ...
            self.PlayerData.job.grade.level = {}
...
function QBCore.Player.Save(source)
    ...
    if PlayerData then
        MySQL.insert('INSERT INTO players (citizenid, cid, license, name, money, charinfo, job, gang, position, metadata, tattoos, furnitures, currentproperty) VALUES (:citizenid, :cid, :license, :name, :money, :charinfo, :job, :gang, :position, :metadata, :tattoos, :furnitures, :currentproperty) ON DUPLICATE KEY UPDATE cid = :cid, name = :name, money = :money, charinfo = :charinfo, job = :job, gang = :gang, position = :position, metadata = :metadata, tattoos = :tattoos, furnitures = :furnitures, currentproperty = :currentproperty', {
...
function QBCore.Player.SaveOffline(PlayerData)
    if PlayerData then
        MySQL.Async.insert('INSERT INTO players (citizenid, cid, license, name, money, charinfo, job, gang, position, metadata, tattoos, furnitures, currentproperty) VALUES (:citizenid, :cid, :license, :name, :money, :charinfo, :job, :gang, :position, :metadata, :tattoos, :furnitures, :currentproperty) ON DUPLICATE KEY UPDATE cid = :cid, name = :name, money = :money, charinfo = :charinfo, job = :job, gang = :gang, position = :position, metadata = :metadata, tattoos = :tattoos, furnitures = :furnitures, currentproperty = :currentproperty', {
            citizenid = PlayerData.citizenid,
            cid = tonumber(PlayerData.cid),
            license = PlayerData.license,
            name = PlayerData.name,
...
function QBCore.Functions.CreatePhoneNumber()
    ...
    while not UniqueFound do
        PhoneNumber = '555 - ' .. QBCore.Shared.RandomInt(4)
        local query = '%' .. PhoneNumber .. '%'
        local result = MySQL.prepare.await('SELECT COUNT(*) as count FROM players WHERE phone LIKE ?', { query })
        if result == 0 then
            UniqueFound = true
        end
    end
